...
def encodeMac(mac_addr_string):
    return bytes.fromhex(mac_addr_string.replace(':', ''))  # python3
    #return mac_addr_string.replace(':', '').decode('hex')

def decodeMac(encoded_mac_addr):
    hex_str = binascii.hexlify(encoded_mac_addr).decode('ascii')
    return ':'.join(hex_str[i:i+2] for i in range(0, len(hex_str), 2))  # python3
...
def encodeNum(number, bitwidth):
    ...
    # pad with zeros and convert hex string to bytes
    hex_str = '0' * (byte_len * 2 - len(num_str)) + num_str
    return binascii.unhexlify(hex_str)  # python3
    # or equivalently: return bytes.fromhex(hex_str)
    #return ('0' * (byte_len * 2 - len(num_str)) + num_str).decode('hex')

def decodeNum(encoded_number):
    return int(binascii.hexlify(encoded_number).decode('ascii'), 16)  # python3
