...
control MyIngress(inout headers hdr,
    ...
    bit <1> in_enq = 1;
    bit <1> in_deq = 0;
    bit <1> in_use_updated_rank = 0; // 1 : to apply Dynamic WFQ
    bit <1> in_force_deq = 0;
    bit <1> in_enable_error_correction = 0; // 1 : to apply Dynamic WFQ
    bit <1> update_flow_rank = 0;
    bit <1> reset_time = 0;

	register<bit<48>>(3) weights;
	register<bit<48>>(3) finish_time;
	register<bit<48>>(1) last_window_ingress_time;
	
// in Dynamic WFQ paper, the service rate v(i) = 1/delta * (queue_occupancy + 1/2 * arrival_Rate_at_the_last_time_window), this experiment is for queue_occupancy only as it is mentioned
// in the P4 working group list, delta = initial_weigths, can be changed to the ratio of delays among flows (require 1 more register / flow), if we took arrival rate into consideration, 
// it will require 1 more register / flow.

	bit <48> weight_value;
	bit <48> finish_time_value;
	bit <48> length_over_weight = 0;
	bit <48> queue_occupancy_value = 0;
	bit <48> queue_occupancy_over_weight = 0;
	bit <48> time_window_period = 1000000; // in microseconds, 1s
	bit <48> last_window_ingress_time_value;
	bit <48> sum_all_weights = 4;
    ...
    apply {
		...
		weights.write(0, (bit<48>)(1)); // in this example, we test 3 flow IDs.
		weights.write(1, (bit<48>)(1));
		weights.write(2, (bit<48>)(2));
		...
		last_window_ingress_time.read(last_window_ingress_time_value, (bit<32>)(in_flow_id));
		queue_occupancy.read(queue_occupancy_value, (bit<32>)(in_flow_id));

		if((bit<48>)(standard_metadata.ingress_global_timestamp) - last_window_ingress_time_value > time_window_period)
		{
			queue_occupancy_value = queue_occupancy_value * weight_value;
			if(queue_occupancy_value >= sum_all_weights)
			{
				floor_extern_obj.floor(queue_occupancy_value, sum_all_weights, queue_occupancy_over_weight);
				weight_value = queue_occupancy_over_weight;
				weights.write((bit<32>)(in_flow_id), weight_value);
				// update_flow_rank = 1; // uncomment this line if you need to apply Dyanmic WFQ
			}
			last_window_ingress_time.write((bit<32>)(in_flow_id), (bit<48>)(standard_metadata.ingress_global_timestamp));
		}
		...
		queue_occupancy.write((bit<32>)(in_flow_id), queue_occupancy_value + (bit<48>)(1));

		my_AR_PIFO.pass_rank_values(level_0_rank,0);
		
		//if(update_flow_rank == 1)
		//{
		//	my_AR_PIFO.pass_updated_rank_values(level_0_rank,in_flow_id,0);
		//}    
