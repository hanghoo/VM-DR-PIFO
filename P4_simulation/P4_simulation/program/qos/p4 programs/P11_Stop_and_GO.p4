...
control MyIngress(inout headers hdr,
	...
	register<bit<48>>(1) frame_begin_time;
	register<bit<48>>(1) frame_end_time;

    bit <48> T_period = 10000; // the length of the time frames
    bit <48> frame_begin_time_reg = 0;
    bit <48> frame_end_time_reg = 0;
    ...
    apply {
		...
		frame_end_time.read(frame_end_time_reg,0);
		
		if((bit<48>)(standard_metadata.ingress_global_timestamp) >= frame_end_time_reg)
		{	
			frame_begin_time.read(frame_begin_time_reg,0);
			frame_begin_time_reg = frame_end_time_reg;
			frame_end_time_reg = frame_begin_time_reg + T_period;

			frame_begin_time.write(0,frame_begin_time_reg);
			frame_end_time.write(0,frame_end_time_reg);
		}
		
		level_0_rank = frame_end_time_reg;
