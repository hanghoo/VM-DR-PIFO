...
control MyIngress(inout headers hdr,
	...
	register<bit<48>>(8) num_received_pkts;

	register<bit<48>>(8) sliding_window_id;

	register<bit<48>>(1) last_reset_time;

	register<bit<48>>(1) sliding_window_id_now;

	bit <48> num_received_pkts_value;
	bit <48> last_reset_time_value;
	bit <48> sliding_window_id_value;
	bit <48> sliding_window_id_now_value;
	bit <48> sliding_window_period = 1000000;
	bit <48> threshold = 3; // maximum number of packets that can be received before characterizing the flow as heavy hitter
	bit <48> low = 1000;
	bit <48> high = 1;
    ...
    apply {
		...
		num_received_pkts.read(num_received_pkts_value, (bit<32>)(in_flow_id));
		last_reset_time.read(last_reset_time_value, 0);
		sliding_window_id.read(sliding_window_id_value, (bit<32>)(in_flow_id));
		sliding_window_id_now.read(sliding_window_id_now_value, 0);

		if((bit<48>)(standard_metadata.ingress_global_timestamp) - last_reset_time_value >= sliding_window_period)
		{
			last_reset_time.write(0, (bit<48>)(standard_metadata.ingress_global_timestamp));
			sliding_window_id_now.write(0, sliding_window_id_now_value + (bit<48>)(1));
		}

		level_0_rank = high;

		if(sliding_window_id_value != sliding_window_id_now_value)
		{
			if(num_received_pkts_value >= threshold)
			{
				level_0_rank = low;
			}
			else
			{
				num_received_pkts_value = 0;
			}
			sliding_window_id.write((bit<32>)(in_flow_id), sliding_window_id_now_value);
		}
		else if (num_received_pkts_value >= threshold)
		{
			level_0_rank = low;
		}

		num_received_pkts.write((bit<32>)(in_flow_id), num_received_pkts_value + (bit<48>)(1));
