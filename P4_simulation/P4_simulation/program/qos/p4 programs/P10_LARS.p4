...
control MyIngress(inout headers hdr,
	...
	bit <48> M_nom = 1; // change these parameters based on the behavior needed (check the LARS paper)
	bit <48> M_dom = 10; // change this parameter
	bit <48> tau = 10; // change this parameter (decay_time_unit)
	register<bit<48>>(1) time_count_till_tau; // current_time_unit
	register<bit<48>>(1024) decayed_value_reg; // this example supports 1024 flows.
	bit <48> time_count_till_tau_var = 1;
	bit <48> decayed_value_reg_var = 1;
	bit <48> decayed_value_temp = 1;
    ...
    apply {
		...
		time_count_till_tau.read(time_count_till_tau_var,0);
		decayed_value_reg.read(decayed_value_reg_var,(bit<32>)(in_flow_id));
		
		if(time_count_till_tau_var == tau)
		{
			time_count_till_tau.write(0,(bit<48>)(0));
			decayed_value_temp = decayed_value_reg_var * M_nom;
			floor_extern_obj.floor(decayed_value_temp, M_dom, decayed_value_reg_var);
		}
		else
		{
			time_count_till_tau.write(0, time_count_till_tau_var + (bit<48>)(1));
		}

		decayed_value_reg_var = decayed_value_reg_var + (bit<48>)hdr.ipv4.options;

		decayed_value_reg.write( (bit<32>)(in_flow_id), decayed_value_reg_var);
		
		level_0_rank = decayed_value_reg_var;
