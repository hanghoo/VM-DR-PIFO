...
control MyIngress(inout headers hdr,
	...
	bit <48> packet_length = 2160; // assume constant packet's length, but it doesn't have to be. 2160 is chosen to repeat the same exp of WFQ.
    ...
    apply {
		...
		weight_value = (bit<48>)hdr.ipv4.options; // assume the weights are sent with "every" packet, if this is not right, we will need a weight "register/flow".
	
		floor_extern_obj.floor(packet_length, weight_value, length_over_weight);
