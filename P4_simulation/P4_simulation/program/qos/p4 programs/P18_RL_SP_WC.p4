...
control MyIngress(inout headers hdr,
	...
	register<bit<48>>(3) number_of_pkt_received;
	register<bit<48>>(3) window_id;

	bit <48> max_number_of_pkts = 4;
	bit <48> max_number_of_sent_pkts_per_window = 36; // number_of_flows * max_possible_num_of_sent_pkts_from_1_flow * maximum_rank_value = 3 * 4 * 3 (high, mid, low),
    ...
    apply {
		...
		if(number_of_pkt_received_value == max_number_of_pkts)
		{
		...
		level_0_rank = window_id_value * max_number_of_sent_pkts_per_window + (bit<48>)hdr.ipv4.options; // the rank can be calculated by any other algorithms, example : by calculating the BW for each flow.		
