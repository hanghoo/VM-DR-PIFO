...
control MyIngress(inout headers hdr,
    ...
    bit <48> stored_pkts_value = 0;

    register<bit<48>>(1) delete_this1;
    register<bit<2>>(1) delete_this2;
    ...
    apply {
        ...
        stored_pkts.read(stored_pkts_value,0);
        stored_pkts_value = stored_pkts_value + (bit<48>)(1);
        stored_pkts.write(0,stored_pkts_value);

	//flow_id_at_level_1 = in_flow_id - 1;
	...
	if(hdr.ipv4.ecn == 3)
	{
		level_0_rank = 1;
		in_flow_id = 1;
        	delete_this2.write(0,1);
	}
	else
	{
		level_0_rank = 2;
		in_flow_id = 2;
        	delete_this2.write(0,hdr.ipv4.ecn);
	}

        delete_this1.write(0,level_0_rank);
        delete_this2.write(0,hdr.ipv4.ecn);

	if(stored_pkts_value > 7500)
	{
		hdr.ipv4.ecn = 3;
	}
