...
void my_scheduler(in T1 in_flow_id, in T1 number_of_levels_used, in T1 in_pred, in T1 in_arrival_time, in T2 in_shaping, in T2 in_enq, in T1 in_pkt_ptr, in T2 in_use_queues_rank, in T2 in_use_updated_rank, in T2 in_force_deq, in T1 in_force_deq_flow_id, in T2 in_enable_error_correction, in T2 reset_time);
...
void pass_updated_rank_values_for_queues ( in T1 rank_value, in T1 queue_id, in T1 level_id);
}
...
control MyIngress(inout headers hdr,
    ...
    apply {
		...
		else
		{
			...
			weights.write(0, (bit<48>)(10)); // service weight of Flow 0	
			weights.write(1, (bit<48>)(20)); // service weight of Flow 1
			weights.write(2, (bit<48>)(20)); // service weight of Flow 2
			weights.write(3, (bit<48>)(20)); // service weight of Flow 3
			weights.write(0, (bit<48>)(30)); // service weight of Flow 5
			...
			floor_extern_obj.floor((in_flow_id-1), (bit<48>)(18), flow_id_at_level_1); // we have 72 hosts, but this example only supports 5 flows.

			// WFQ rank calculations
			finish_time.read(finish_time_value, (bit<32>)(flow_id_at_level_1));
			weights.read(weight_value, (bit<32>)(flow_id_at_level_1));
	
			floor_extern_obj.floor((bit<48>)hdr.ipv4.options, weight_value, length_over_weight);
			
			floor_extern_obj.floor(standard_metadata.ingress_global_timestamp, (bit<48>)(10000), arrival_time);

			if(finish_time_value > arrival_time)
			{
				level_0_rank = finish_time_value + length_over_weight;
			}
			else
			{
				level_0_rank = arrival_time + length_over_weight;
			}

			finish_time_value = level_0_rank;
			finish_time.write((bit<32>)(flow_id_at_level_1), finish_time_value);

			my_DR_PIFO.pass_rank_values(level_0_rank,0);

            my_DR_PIFO.my_scheduler(flow_id_at_level_1, number_of_levels_used, in_pred, in_pkt_ptr, in_shaping, in_enq, in_pkt_ptr, in_use_queues_rank, in_use_updated_rank, in_force_deq, in_force_deq_flow_id, in_enable_error_correction, reset_time);
	}        
        if (hdr.ipv4.isValid()) {   
           ipv4_lpm.apply();
        }  
