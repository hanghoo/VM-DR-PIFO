...
control MyIngress(inout headers hdr,
    ...
    apply {
		...
		level_0_rank = (bit<48>)hdr.ipv4.diffserv + (bit<48>)(standard_metadata.ingress_global_timestamp);

		in_pred_micro_second =  (bit<48>)hdr.ipv4.options + (bit<48>)(standard_metadata.ingress_global_timestamp); // all in millisecond

		floor_extern_obj.floor(in_pred_micro_second, (bit<48>)(1000), in_pred); // seconds
...
control MyEgress(inout headers hdr,
    ...
    apply { 
		...
		level_0_rank = (bit<48>)hdr.ipv4.diffserv + (bit<48>)(standard_metadata.ingress_global_timestamp); // can be removed, and extracted from the Traffic Manager.

		if((bit<32>)level_0_rank < (bit<32>)(standard_metadata.egress_global_timestamp))
		{
			hdr.ipv4.options = (bit<32>)(standard_metadata.egress_global_timestamp) - (bit<32>)level_0_rank;
		}
		else
		{
			hdr.ipv4.options = (bit<32>)0;
		}
