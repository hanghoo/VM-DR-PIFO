...
control MyIngress(inout headers hdr,
    ...
    apply {
		...
		service_count.read(level_0_rank,(bit<32>)in_flow_id);
		level_0_rank = level_0_rank + (bit<48>)(1); 		// assuming all packets are the same size, if not, replace the (+1) by the Packet Size Metadata hdr.ipv4.totalLen
		service_count.write((bit<32>)in_flow_id,level_0_rank);
