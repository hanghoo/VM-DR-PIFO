...
control MyIngress(inout headers hdr,
    ...
    register<bit<48>>(1024) flows_min_rank; // this example support 1024 flows
    bit <48> in_flow_id = 0;
    ...
    apply {
        ...
        flows_min_rank.read(level_0_rank,(bit<32>)in_flow_id);

		if((level_0_rank == 0)||((bit<48>)(hdr.ipv4.options) < level_0_rank))
		{
			level_0_rank = (bit<48>)(hdr.ipv4.options);
			new_rank_flow_id = in_flow_id;
			flow_new_rank = level_0_rank;
			update_flow_rank = 1;
			flows_min_rank.write((bit<32>)in_flow_id,level_0_rank);
		}
		...
		if((lowest_rank_value > level_0_rank) || (lowest_rank_value==0))
		{
			lowest_rank_value = level_0_rank;
