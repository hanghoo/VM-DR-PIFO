...
def measure_bandwidth_allocation(outputs_dir, start_time=None, end_time=None, window_size=10):
    ...
    if total_rate_all > 0:
        ...
        # Expected allocation (based on quantums = [20000, 10000, 2000])
        #expected = [62.5, 31.25, 6.25]
        #print("\nExpected allocation (quantums 10:5:1):")
        expected = [76.92, 19.23, 3.85]  # 20:5:1 (40000:10000:2000)
        print("\nExpected allocation (quantums 20:5:1):")
        for flow_id in range(3):
            diff = abs(percentage - expected[flow_id])
            print(f"  Flow {flow_id}: {expected[flow_id]:.2f}% (actual: {percentage:.2f}%, diff: {diff:.2f}%)")
